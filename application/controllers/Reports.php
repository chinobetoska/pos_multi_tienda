...
class Reports extends CI_Controller
{
        ...
        if ($orderRows > 0) {
            ...
            for ($od = 0; $od < count($orderData); ++$od) {
                ...
                if($order_type == "1") {
	                ...
	                $objPHPExcel->getActiveSheet()->setCellValue("D$jj", "$payment_name_list");
    ...
    public function exportSoldByModelReport() {
	    if(isset($_GET["url_start"]) && isset($_GET["url_end"])) {
		    $url_start 		= strip_tags($this->input->get("url_start"));
		    $url_end 		= strip_tags($this->input->get("url_end"));
	        ...
	        $lang_sold_by_model_title 		= $this->lang->line("sold_by_product_report_title");
	        $lang_product_code 				= $this->lang->line("product_code");
	        $lang_product_name 				= $this->lang->line("product_name");
	        $lang_product_cat_name 			= $this->lang->line("product_category");
	        $lang_total_sold_qty 			= $this->lang->line("sold_by_product_sold_qty");
	        $lang_total 					= $this->lang->line("total");
	        $lang_no_match_found 			= $this->lang->line("no_match_found");
	        
		    $objPHPExcel->setActiveSheetIndex(0)->mergeCells('A1:D1');
			$objPHPExcel->getActiveSheet()->setCellValue('A1', "$lang_sold_by_model_title");
	        ...
	        $objPHPExcel->getActiveSheet()->setCellValue('A2', "$lang_product_code");
	        $objPHPExcel->getActiveSheet()->setCellValue('B2', "$lang_product_name");
	        $objPHPExcel->getActiveSheet()->setCellValue('C2', "$lang_product_cat_name");
	        $objPHPExcel->getActiveSheet()->setCellValue('D2', "$lang_total_sold_qty");
			...
			if(count($pcodeArray) > 0) {
				...
				for($p = 0; $p < count($pcodeArray); $p++) {
	                ...
	                $objPHPExcel->getActiveSheet()->setCellValue("B$jj", "$pname");
	                $objPHPExcel->getActiveSheet()->setCellValue("C$jj", "$pcat_name");
	                $objPHPExcel->getActiveSheet()->setCellValue("D$jj", "$total_sold_qty");
					...
					$objPHPExcel->getActiveSheet()->getRowDimension("$jj")->setRowHeight(25);
					
					$all_total_qty 		+= $total_sold_qty;
				...
				$objPHPExcel->getActiveSheet()->setCellValue("A$jj", "$lang_total");
				$objPHPExcel->getActiveSheet()->setCellValue("D$jj", "$all_total_qty");
				...
				$objPHPExcel->setActiveSheetIndex(0)->mergeCells("A$jj:D$jj");
				$objPHPExcel->getActiveSheet()->setCellValue("A$jj", "$lang_no_match_found");
		    ...
		    // Redirect output to a client’s web browser (Excel5)
	        header('Content-Type: application/vnd.ms-excel');
	        header('Content-Disposition: attachment;filename="Sold_By_Model_Report.xls"');
	        header('Cache-Control: max-age=0');
	        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
	        $objWriter->save('php://output');
	    }
